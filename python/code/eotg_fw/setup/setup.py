...
def variable_setup():
    ...
    ret_dss, ret_dsv, ret_mpu, ret_pump, ret_heater, ret_wifi = {(k,v) for (k,v) in zip(tu_dss, row_dss)},  {(k,v) for (k,v) in zip(tu_dsv, row_dsv)},  {(k,v) for (k,v) in zip(tu_mpu, row_mpu)},  {(k,v) for (k,v) in zip(tu_rly, row_pump)}, {(k,v) for (k,v) in zip(tu_rly, row_heater)},  {(k,v) for (k,v) in zip(tu_wifi, row_wifi[0])},
